...
namespace
{
    ...
    int wavPackWriteBlock(void *id, void *data, int32_t length)
    {
        ...
        const size_t written =
            fwrite(data, 1, static_cast<size_t>(length), encoder->file);
                                     ...
                                     void **outEncoder)
    {
        ...
        encoder->context =
            WavpackOpenFileOutput(wavPackWriteBlock, encoder, nullptr);
        ...
        config.channel_mask =
            static_cast<int32_t>(captureWavPackChannelMask(channels));
        if (encoder->isFloat)
        {
            config.float_norm_exp = 127;
        }

        if (!WavpackSetConfiguration64(encoder->context, &config, -1,
                                       nullptr) ||
                                    ...
                                    ma_uint32 frameCount)
    {
        ...
        if (!WavpackPackSamples(encoder->context,
                                encoder->sampleBuffer.data(), frameCount))
        {
            encoder->failed = true;
            return false;
        }
