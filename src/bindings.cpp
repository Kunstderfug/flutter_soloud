...
FFI_PLUGIN_EXPORT void listPlaybackDevices(char **devicesName, int **deviceId,
  ...
  for (int i = 0; i < (int)d.size(); i++) {
    ...
    size_t nameLength = strlen(d[i].name);
...
startCapture(char *path, unsigned int sampleRate, unsigned int channels,
  ...
  PlayerErrors result = player.get()->startCapture(
      ...
      mirrorFormat, mirrorBitsPerSample,
      &info);
  if (result == noError) {
...
FFI_PLUGIN_EXPORT enum PlayerErrors
startCaptureAndPlay(char *path, unsigned int soundHash, unsigned int busId,
                    unsigned int sampleRate, unsigned int channels,
                    unsigned int bufferSizeFrames, float volume, float pan,
                    double startAtSeconds, bool looping,
...
FFI_PLUGIN_EXPORT enum PlayerErrors playWithLoopPoints(
  ...
  if (player.get() == nullptr || !player.get()->isInited())
    return backendNotInited;
  return player.get()->play(soundHash, *handle, busId, volume, pan, paused,
...
///
/// [time] is in seconds, or zero to use the natural stream end.
...
FFI_PLUGIN_EXPORT PlayerErrors play3dWithLoopPoints(
  ...
  return player.get()->play3d(soundHash, *handle, posX, posY, posZ, velX, velY,
                              velZ, volume, paused, busId, looping,
